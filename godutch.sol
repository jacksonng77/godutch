...
contract godutch {
	...
	*/
	function godutch(bytes32[] friendsNames) {
	...
	*/
	function totalContrib(bytes32 thefriend) constant returns (uint) {
	...
	*/
	function contribute(bytes32 thefriend, uint amount) {
		uint index = indexOfFriend(thefriend);
		if (index == uint(-1)) throw; //friend doesn't exist
	...
	*/
	function friendDetails(address user) constant returns (uint, uint[]) {
	...
	*/
	function indexOfFriend(bytes32 thefriend) constant returns (uint) {
		for(uint i = 0; i < friendList.length; i++) {
	...
	*/
	function whatIsOurPool() constant returns (uint) {
		return pool;
	}
  
	function allFriends() constant returns (bytes32[]) {
		return friendList;
	}
}